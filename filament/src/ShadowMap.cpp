...
void ShadowMap::computeShadowCameraDirectional(
    ...
    if (mHasVisibleShadows) {
        ...
        if (USE_LISPSM) {
            ...
            slog.d << mTexelSizeWs << "\n"
            << mTexelSizeWs/texelSizeWorldSpace(St) << "\n"
            << texelSizeWorldSpace(MbMt * F * W) << "\n"
            << texelSizeWorldSpace(MbMt * F * Wp) << "\n"
            << io::endl;
...
float ShadowMap::texelSizeWorldSpace(const mat4f& worldToShadowTexture) const noexcept {
    ...
    const float3 Jx = length(p1 - p0);
    const float3 Jy = length(p2 - p0);
